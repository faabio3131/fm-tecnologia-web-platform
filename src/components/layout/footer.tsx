...
export function Footer({ premiumHome = false }: { premiumHome?: boolean }) {
  return (
    <footer className={`site-footer${premiumHome ? " site-footer--premium-home" : ""}`}>
          ...
          <div className="footer-grid">
            {groups.map((group) => (
              <section key={group.title}>
                ...
                ) : <p className="muted">Informações em preparação.</p>}
              </section>
            ))}
          </div>
        </div>
        <div className="footer-bottom">
          <span>© {new Date().getFullYear()} FM Tecnologia.</span>
          <span>Construído com propósito no Brasil.</span>
        </div>
      </div>
    </footer>
  );
